🛠 TUTO | MEGA DRIVE MINI HACK | Monta tu PACK de 32Gb con 28 SISTEMAS | PSX PSP NEOGEO MAME N64 #04 🛠
✓https://youtu.be/a_jHKR53SSU

------------------------------------------------------------

PARA PROBAR TODO ESTE "BASIC PACK 2.0 de 32Gb" tal y como se ve en este vídeo: ✓https://youtu.be/ys_5WKTLNY4

Se ha de instalar siguiendo los tutoriales al respecto el EMULATION STATION en la Mega Drive MINI, y acto seguido copiar y reescribir si lo pide, todo los contenidos de este PACK en el directorio RAIZ (Principal) de la memoria externa. 
TUTORIAL INSTALACION EMULATION STATION: ✓https://youtu.be/ys_5WKTLNY4

Las ruta dentro del pendrive debe quedar así:
X:\project_lunar
X:\mega_drive_saves
X:\swap

El "Mega Drive Mini ES BASIC PACK 2.0" cabe en una memoria USB de 32Gb o superior, e incluye la siguiente selección de sistemas:
MEGA DRIVE, MEGA-CD, 32X, MASTER SYSTEM, GameGear, NES y SNES.

Para jugar a algunos sistemas es necesario utilizar algún mando con más botones y gatillos, como el de Playstation.
En el caso de los sistemas PC viejunos, es muy recomendable y en algunos casos imprescindible, conectar un teclado USB.

Estará disponible para probar en:
Canal Telegram Consolas MINI: 
✓https://t.me/snesminiroms

------------------------------------------------------------

Agradecimientos al equipo de "Best of the Best (BOB)" ya que me he servido de su maravillosa selección de sistemas ya escrapeados y con información de cada título para la elaboración de este "BASIC Pack" para nuestra Mega Drive Mini.
Tan solo he tenido que eliminar algunas cosas y realizar pequeños cambios o modificaciones de los romsets para que todo funcione como es debido en la MINI de SEGA.

✓https://t.me/BOB_Retropie_Windows


▼▬▼▬▼▬ LINKS UTILES DE LA SCENE MEGA DRIVE MINI▬▼▬▼▬▼

✓https://modmyclassic.com/project-lunar-cores/
✓https://modmyclassic.com/project-lunar-mods/
✓https://modmyclassic.com/project-lunar/#Where_to_Download
✓https://modmyclassic.com/project-lunar/
✓https://docs.google.com/spreadsheets/d/1lW8ViyN_Sg0hrXCAqGgX0UEYwaWMhrcbJ_GzzNfTNlA/edit

https://github.com/PaPer-DJ/PaPer_DJ-MEGA-DRIVE-Classic-SCENE


Canal Telegram Consolas MINI: 
https://t.me/snesminiroms

------------------------------------------------------------------

▼▬▼▬▼▬▼▬ DISCLAIMER ▬▼▬▼▬▼▬▼

Info/Tuto/Review/Mod/Recursos By PaPer_DJ

- Este vídeo y todo el material recopilado, modificado y expuesto ESTÁN HECHOS CON PROPÓSITOS EDUCATIVOS únicamente.
- No promuevo la piratería, recomiendo comprar juegos que estén en el mercado, para apoyar a las desarrolladoras.
- Al usar "ROMS" o "ISOS" se supone que se posee la licencia original de dichos juegos. Descargar juegos sin licencia es ilegal.